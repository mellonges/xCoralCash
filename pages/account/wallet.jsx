...
const Wallet = () => {
  ...
  const [paymentMethods, setPaymentMethids] = useState();
  ...
  const loadMoreTransfers = (order = 1, lastTime) => {
    ...
    getUserData([
      {
        ...
        params: {
          ...
          lastTransferDate: lastTime,
        },
      },
    ])
      .then((res) => res.data.payload.processedData)
      .then((res) => {
        let item = res.find((item) => item.type === "transfers");

        if (item) setTransfers(item.transfers);

        if (order === -1) {
          if (page !== 1) setHasMoreBefore(item.hasMore);
          else setHasMoreBefore(false);

          setHasMoreAfter(true);
        } else if (order === 1) {
          setHasMoreAfter(item.hasMore);
          if (page !== 1) setHasMoreBefore(true);
        }
      })
      .finally(() => {
        setLoading(false);
  ...
  useEffect(() => {
    if (pageRef.current !== page) {
      ...
      if (transfers && transfers.length)
        if (direction === -1) lastTime = transfers[0].timestamp;
        else if (direction === 1)
          lastTime = transfers.slice(-1).pop().timestamp;

      loadMoreTransfers(direction, lastTime);
    }
  }, [page]);
  ...
  useEffect(() => {
    ...
    getUserData([
      ...
      .then((res) => {
        res.map((item) => {
          if (item.type === "payment_methods")
            setPaymentMethids(item.paymentMethods);
        });
      })
      .catch((error) => console.error(error));

    loadMoreTransfers(1, moment().unix());
  }, []);

  return (
    <AccountLayout user={user} pageTitle={"Wallet"}>
      <div className={styles.Wallet}>
        <h1 className={styles.pageTitle}>Wallet</h1>

        <WalletTopBlock paymentMethods={paymentMethods} />
